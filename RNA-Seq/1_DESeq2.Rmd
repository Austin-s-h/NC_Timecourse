...
This analysis was performed on a 64 core machine running Ubuntu 18.04 LTS. Certain parameters may need to be updated based on your operating system and available CPUs.

```{r Loading Libraries, message=FALSE, include=TRUE}

## First specify the packages of interest
packages = c("tidyr", "reshape2",
             "ggplot2", "scales",
             "gridExtra","ggthemes",
             "ggrepel","dplyr",
             "readr","ggsci",
...
bioc_packages <- c("edgeR","Rsubread","DESeq2","org.Gg.eg.db",
                   "biomaRt","DOSE","org.Hs.eg.db","clusterProfiler")
...
dir.create(file.path("./Exports"))
```

### Read in the results of the featureCounts
Data processing pipeline is executed by `process_Chicken_RNASeq.sh`
...
featureCounts <- read.delim("./Imports/featureCounts.txt", comment.char="#")
...
write.csv(counts(dds,normalized=F), file = "./Exports/Raw_RNA-Seq_Counts.csv", row.names = T)
...
g <- ggplot(pcaAll, aes(x=PC1, y=PC2, color=Time, shape = Condition)) + 
  ...
  scale_color_npg() +
  ggtitle("pcaALL") +
  labs(x = paste0("PC1 ", round(pcaAllVar[1]*100)," %"),
       y = paste0("PC2 ", round(pcaAllVar[2]*100)," %")) +
...
DESeq2_Analyze <- function(timepoint = "", dds, genes.universe = genes.universe, versusWE = TRUE, second_timepoint = "", Padj = 0.1, L2fc = 1, testMF = TRUE, testBP = TRUE, plotVolc = TRUE, labelPadj = 0.001) {
  ...
  if (plotVolc == TRUE){
      ...
      ggsave(filename = paste0("./Exports/",restest_title,"_labeled.pdf"), plot = g, width = 4, height = 4, device = "pdf")
...
DESeq2_Analyze("6", dds, genes.universe = genes.universe, Padj = 0.05, L2fc = 1.5, testMF = FALSE, testBP = FALSE, plotVolc = TRUE, labelPadj = 0.001)
DESeq2_Analyze("8", dds, genes.universe = genes.universe, Padj = 0.05, L2fc = 1.5, testMF = FALSE, testBP = FALSE, plotVolc = TRUE, labelPadj = 0.00001)
DESeq2_Analyze("10", dds, genes.universe = genes.universe, Padj = 0.05, L2fc = 1.5, testMF = FALSE, testBP = FALSE, plotVolc = TRUE, labelPadj = 0.001)
DESeq2_Analyze("12", dds, genes.universe = genes.universe, Padj = 0.05, L2fc = 1.5, testMF = FALSE, testBP = FALSE, plotVolc = TRUE, labelPadj = 0.0001)
DESeq2_Analyze("14", dds, genes.universe = genes.universe, Padj = 0.05, L2fc = 1.5, testMF = FALSE, testBP = FALSE, plotVolc = TRUE, labelPadj = 0.0005)
DESeq2_Analyze("16", dds, genes.universe = genes.universe, Padj = 0.05, L2fc = 1.5, testMF = FALSE, testBP = FALSE, plotVolc = TRUE, labelPadj = 0.001)
```
