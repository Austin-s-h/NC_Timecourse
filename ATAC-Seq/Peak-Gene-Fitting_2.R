...
compareGeneToPeaks <-
  function(gene = "") {
    ...
    peaks_of_interest <-
      peaks_of_interest[peaks_of_interest %in% impulse_obj_ATAC@vecAllIDs]
    ...
    for (peak in peaks_of_interest) {
      peak_fit <-
        getDfFit(
          vecGeneIDs = peak,
          objectImpulseDE2 = impulse_obj_ATAC,
          boolCaseCtrl = F,
          boolSimplePlot = T
        )
      peak_fit <- peak_fit[,2]
...
RunMultipleGenes <- function(genelist = "") {
  ...
  if (length(genelist) > 5000) {
    ...
    results.df$Symbol <- lookup(results.df$gene, key.match = as.data.frame(GenesUniverse[, c(1, 2)]))
  }
