...
```{r}
data_dir <- "/data/Austin/workdir/NC_Timecourse_Data/Oct4-Sox2-OE-ATAC/BAM/"
...
ATAC_OE <-
  dba.peakset(
    ...
    treatment = "A",
    replicate = 1,
    bamReads = paste0(data_dir,"ATAC_Oct4-Sox2-Control_1_togalGal6.bam")
    ,
...
ATAC_OE <-
  dba.peakset(
    ...
    treatment = "A",
    replicate = 1,
    bamReads = paste0(data_dir,"ATAC_Oct4-Sox2-OE_1_togalGal6.bam")
    ,
    peak.caller = "narrowpeak"
  )
ATAC_OE <-
  dba.peakset(
    ...
    treatment = "A",
    replicate = 2,
    bamReads = paste0(data_dir,"ATAC_Oct4-Sox2-Control_2_togalGal6.bam")
    ,
    peak.caller = "narrowpeak"
  )
ATAC_OE <-
  dba.peakset(
    ...
    treatment = "A",
    replicate = 2,
    bamReads = paste0(data_dir,"ATAC_Oct4-Sox2-OE_2_togalGal6.bam")
    ,
    peak.caller = "narrowpeak"
  )
...
Differential_regions_up = subset(Differential_regions, Differential_regions$Fold > 0)
...
export(OE_regions_up, con = "Exports/SOX2-OCT4_OE_regions_up.bed", format = "BED")
...
dds_results.df <- as.data.frame(dds_results)
...
Scan peaks identified as enriched in OCT4/SOX2 overexpression for dimer motifs
```{bash}
# Running FIMO for TFAP2A on the ESC vs NC Specific dual-motif locations
# First get FASTA for motif locations.
bedtools getfasta -fi /data/Austin/workdir/genome/galGal6/dna/Gallus_gallus.GRCg6a.dna.toplevel.fa -bed ./Exports/SOX2-OCT4_OE_regions_up.bed > ./Exports/SOX2-OCT4_OE_regions_up.fa

# Now FIMO.
fimo --thresh 1e-3 -o ./Exports/SOX2-OCT4_OE_regions_up_dimer ./Imports/MA0142.1.meme ./Exports/SOX2-OCT4_OE_regions_up.fa
```
...
library(readr)
library(stringr)
OE_Fimo_Results <- read_delim(file = "Exports/SOX2-OCT4_OE_regions_up_dimer/fimo.tsv", delim = "\t")

OE_Fimo_Results$sequence_name <- gsub(x = OE_Fimo_Results$sequence_name, pattern = ":", replacement = "_")
OE_Fimo_Results$sequence_name <- gsub(x = OE_Fimo_Results$sequence_name, pattern = "-", replacement = "_")

OE_Fimo_Results_df <- (data.frame(chr = str_split(OE_Fimo_Results$sequence_name, pattern = "_", simplify = T)[,1],
                                    start = as.numeric(str_split(OE_Fimo_Results$sequence_name, pattern = "_", simplify = T)[,2]),
                                    stop = as.numeric(str_split(OE_Fimo_Results$sequence_name, pattern = "_", simplify = T)[,3])))

OE_Fimo_Results_df <- OE_Fimo_Results_df[!is.na(OE_Fimo_Results_df$chr),]

OE_Fimo_Results_gr <- makeGRangesFromDataFrame(OE_Fimo_Results_df)

OE_Increased_Regions_gr <- makeGRangesFromDataFrame(Differential_regions_up)

summary(OE_Fimo_Results_gr %over% OE_Increased_Regions_gr)

```

Overlap of 5C (Overexpression peaks) with Shared OCT4/SOX2 Peaks from CR

```{bash}
bedtools shuffle -i ASH_Shared_Sox2_Oct4_peaks_Filt.bed -g /data/Austin/workdir/genome/galGal6/annotation/gg6_2.chrom.sizes > shuffled_matched_shared_OCT4_SOX2_peaks.bed
```

```{r}
library(rtracklayer)
Shared_dimer_peaks <- import.bed(con = "../CR/Exports/ASH_Shared_Sox2_Oct4_peaks_Filt.bed")
random_peaks <- import.bed(con = "../CR/Exports/shuffled_matched_shared_OCT4_SOX2_peaks.bed")

summary(OE_Increased_Regions_gr %over% Shared_dimer_peaks)
# 59
summary(OE_Increased_Regions_gr %over% random_peaks)
# 8

# 7.4 fold enrichment compared to random chance.
